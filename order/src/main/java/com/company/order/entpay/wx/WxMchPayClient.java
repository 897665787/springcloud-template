package com.company.order.entpay.wx;
...
@Slf4j
@Component
public class WxMchPayClient extends EnterprisePaymentClient {
    ...
    protected Object requestPayInfo(PayParams payParams) {
		...
		// 私有参数
		EntPayRequest request = EntPayRequest.newBuilder().build();
		request.setMchAppid(payConfig.getAppId());
		request.setMchId(mchConfig.getMchId());
		request.setPartnerTradeNo(payParams.getOutTradeNo());
		request.setOpenid(payParams.getOpenid());
		request.setCheckName("NO_CHECK");
		request.setAmount(payParams.getAmount().multiply(new BigDecimal(100)).intValue());
