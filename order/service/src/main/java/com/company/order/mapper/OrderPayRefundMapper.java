...
public interface OrderPayRefundMapper extends BaseMapper<OrderPayRefund> {
	...
	@Select("select * from order_pay_refund where refund_order_code = #{refundOrderCode}")
	OrderPayRefund selectByRefundOrderCode(@Param("refundOrderCode") String refundOrderCode);

	@Select("select sum(amount) from order_pay_refund where order_code = #{orderCode} and status in('apply','success')")
	BigDecimal sumRefundAmount(@Param("orderCode") String orderCode);

	@Select("select * from order_pay_refund where order_code = #{orderCode}")
