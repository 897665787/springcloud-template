...
public interface AliPayMapper extends BaseMapper<AliPay> {
	...
	@Select("select * from bu_ali_pay where out_trade_no = #{outTradeNo}")
	AliPay selectByOutTradeNo(@Param("outTradeNo") String outTradeNo);

	@Update("update bu_ali_pay set notify_data = #{notifyData},notify_status = 1 where out_trade_no = #{outTradeNo} and notify_status != 4")
	int updateByOutTradeNo(@Param("outTradeNo") String outTradeNo, @Param("notifyData") String notifyData);

	@Update("update bu_ali_pay set notify_status = #{notifyStatus} where out_trade_no = #{outTradeNo} and notify_status = #{oldNotifyStatus}")
