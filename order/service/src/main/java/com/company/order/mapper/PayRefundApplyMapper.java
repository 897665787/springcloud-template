...
public interface PayRefundApplyMapper extends BaseMapper<PayRefundApply> {
	...
	@Select("select * from pay_refund_apply where order_code = #{orderCode}")
	PayRefundApply selectByOrderCode(@Param("orderCode") String orderCode);

	@Select("select * from pay_refund_apply where old_order_code = #{oldOrderCode} order by id desc limit 1")
	PayRefundApply selectLastByOldOrderCode(@Param("oldOrderCode") String oldOrderCode);

	@Select("update pay_refund_apply set refund_status = #{refundStatus.code},remark = #{remark} where id = #{id}")
	...
	@Select("select id from pay_refund_apply where refund_status = #{noRefundStatus.code} and verify_status in(#{passStatus.code},#{rejectStatus.code})")
	...
	@Select("select * from pay_refund_apply where old_order_code = #{oldOrderCode}")
