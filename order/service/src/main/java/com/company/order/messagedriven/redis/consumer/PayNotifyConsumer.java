	...
	public MessageListener commonMessageListener() {
		return new MessageListener() {
			...
			public void onMessage(org.springframework.data.redis.connection.Message message, byte[] pattern) {
				...
				com.company.framework.messagedriven.redis.utils.ConsumerUtils.handleByStrategy(body, strategyName, paramsClass);
			}
		};
	}

	@Bean
	public Object registerCommonConsumer(RedisMessageListenerContainer container, MessageListener commonMessageListener, MessagedrivenProperties messagedrivenProperties) {
		String channel = String.format("%s:%s", messagedrivenProperties.getExchange().getDirect(), Constants.QUEUE.PAY_NOTIFY.KEY);
