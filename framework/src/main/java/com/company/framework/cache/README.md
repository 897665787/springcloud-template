...
缓存模块是基于Spring Cache抽象的一套统一缓存接口和多种缓存实现，支持Redis、Guava本地缓存、组合缓存等多种缓存策略。该模块提供了统一的[ICache](ICache.java)接口，便于在不同缓存实现之间切换。
...
#### CacheManager缓存
基于Spring CacheManager的缓存实现，可与Spring的[@Cacheable](annotation/Cacheable.java)等注解配合使用。
...
该缓存模块具有良好的扩展性，可以轻松添加新的缓存实现：

1. 实现[ICache](ICache.java)接口
2. 在[CacheAutoConfiguration](CacheAutoConfiguration.java)中添加相应的Bean定义
3. 在配置文件中启用新的缓存实现
