...
public class WeixinMpService implements LoginService {
	...
	public LoginResult login(String ignore1, String ignore2, String wxcode) {
		...
		// 根据openid查询user_id
		Integer userId = oauthTool.getUserIdCertificate(UserOauthEnum.IdentityType.WX_OPENID_MP, openid).getUserId();
		if (userId == null) {
			// 根据unionid查询user_id
			...
			if (userId != null) {
				...
				oauthTool.bindOauth(userId, UserOauthEnum.IdentityType.WX_OPENID_MP, openid, wxcode);
			}
		} else {
		...
		if (userId == null) {
			...
			binds.add(
					new BindUserOauth().setIdentityType(UserOauthEnum.IdentityType.WX_OPENID_MP).setIdentifier(openid));
			binds.add(
