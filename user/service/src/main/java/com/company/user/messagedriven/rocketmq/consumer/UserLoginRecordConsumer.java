...
@RocketMQMessageListener(
        topic = BroadcastConstants.USER_LOGIN.EXCHANGE,
        consumerGroup = BroadcastConstants.USER_LOGIN.LOGIN_RECORD_QUEUE
)
@Slf4j
@Conditional(RocketMQAutoConfiguration.RocketMQCondition.class)
public class UserLoginRecordConsumer implements RocketMQListener<MessageExt> {
    @Override
    public void onMessage(MessageExt messageExt) {
        Map<String, String> properties = messageExt.getProperties();
        String jsonStrMsg = new String(messageExt.getBody(), StandardCharsets.UTF_8);

        properties.put(HeaderConstants.HEADER_STRATEGY_NAME, StrategyConstants.LOGINRECORD_STRATEGY);
