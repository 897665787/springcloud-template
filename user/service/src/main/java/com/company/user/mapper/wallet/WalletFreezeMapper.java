...
public interface WalletFreezeMapper extends BaseMapper<WalletFreeze> {
	...
	@Select("select * from wallet_freeze where unique_code = #{uniqueCode} limit 1")
	WalletFreeze selectByUniqueCode(@Param("uniqueCode") String uniqueCode);

	@Update("update wallet_freeze set status = 2 where unique_code = #{uniqueCode} and status = 1")
	Integer update2Use(@Param("uniqueCode") String uniqueCode);
	
	@Update("update wallet_freeze set status = 3 where order_code = #{orderCode} and status = 1")
	Integer update2Return(@Param("orderCode") String orderCode);
	
	@Select("select sum(freeze_amount) from wallet_freeze where wallet_id = #{walletId} and status = 1")
	BigDecimal sumFreezeAmount(@Param("walletId") Integer walletId);

	@Select("select * from wallet_freeze where order_code = #{orderCode}")
