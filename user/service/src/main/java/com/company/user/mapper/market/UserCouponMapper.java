...
public interface UserCouponMapper extends BaseMapper<UserCoupon> {
	...
	@Select("select * from user_coupon where user_id = #{userId} and status = #{status} order by id desc")
	List<UserCoupon> selectByUserIdStatus(@Param("userId") Integer userId, @Param("status") String status);

	@Update("update user_coupon set status = #{newStatus} where id = #{id} and status = #{oldStatus}")
