...
public interface WalletIncomeUseRecordMapper extends BaseMapper<WalletIncomeUseRecord> {
	...
	@Select("select * from wallet_income_use_record where wallet_id = #{walletId} and status in(1,2) order by invalid_time asc,id asc")
	List<WalletIncomeUseRecord> select2Use(@Param("walletId") Integer walletId);

	@Update("update wallet_income_use_record set unused_amount = unused_amount - #{usedAmount},status = #{status.code},remark = #{remark} where id = #{id} and status in(1,2) and unused_amount >= #{usedAmount}")
	...
	@Select("select id from wallet_income_use_record where invalid_time < now() and status in(1,2) limit #{limit}")
	List<Integer> selectId4Expire(@Param("limit") Integer limit);

	@Update("update wallet_income_use_record set status = #{status.code} where id = #{id} and status in(1,2) and invalid_time < now()")
	Integer update4Expire(@Param("status") WalletRecordEnum.IncomeRecordUseStatus status, @Param("id") Integer id);

	@Select("select * from wallet_income_use_record where wallet_record_id = #{walletRecordId} limit 1")
