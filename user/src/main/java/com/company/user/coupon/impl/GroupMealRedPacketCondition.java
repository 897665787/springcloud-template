...
public class GroupMealRedPacketCondition implements UseCondition {
	...
	public boolean canSee(SeeParam seeParam) {
		...
		if (!canSee) {
			log.info("{}条件不满足,当前不是外卖团餐红包:{}", seeParam.getUserCouponId(), JSON.toJSONString(runtimeAttach));
			return false;
		}
		return true;
	}
	...
	public MatchResult canUse(UseParam useParam) {
		...
		if (!canUse) {
			log.info("{}条件不满足,当前不是外卖团餐红包:{}", useParam.getUserCouponId(), JSON.toJSONString(runtimeAttach));
