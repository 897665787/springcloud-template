...
public class SendCouponConsumer {
	...
	// 优惠券到账提醒、优惠券发放通知
	@RabbitListener(
			bindings = @QueueBinding(value = @Queue(value = FanoutConstants.SEND_COUPON.SUBSCRIBE_RECEIVE_QUEUE, durable = "false", autoDelete = "true"),
					exchange = @Exchange(value = FanoutConstants.SEND_COUPON.EXCHANGE, type = ExchangeTypes.FANOUT, durable = "false", autoDelete = "true")))
	...
	// 优惠券使用提醒
	@RabbitListener(
			bindings = @QueueBinding(value = @Queue(value = FanoutConstants.SEND_COUPON.SUBSCRIBE_TOUSE_QUEUE, durable = "false", autoDelete = "true"),
					exchange = @Exchange(value = FanoutConstants.SEND_COUPON.EXCHANGE, type = ExchangeTypes.FANOUT, durable = "false", autoDelete = "true")))
	...
	// 优惠券过期提醒
	@RabbitListener(
			bindings = @QueueBinding(value = @Queue(value = FanoutConstants.SEND_COUPON.SUBSCRIBE_EXPIRE_QUEUE, durable = "false", autoDelete = "true"),
					exchange = @Exchange(value = FanoutConstants.SEND_COUPON.EXCHANGE, type = ExchangeTypes.FANOUT, durable = "false", autoDelete = "true")))
