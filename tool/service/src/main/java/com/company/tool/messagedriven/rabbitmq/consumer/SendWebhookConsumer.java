...
public class SendWebhookConsumer {
	...
	@RabbitListener(
			bindings = @QueueBinding(value = @Queue(value = Constants.QUEUE.SEND_WEBHOOK.NAME),
					exchange = @Exchange(value = "${messagedriven.exchange.direct}"), key = Constants.QUEUE.SEND_WEBHOOK.KEY))
