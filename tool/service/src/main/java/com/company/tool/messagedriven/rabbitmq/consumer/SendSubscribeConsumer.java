...
public class SendSubscribeConsumer {
	...
	@RabbitListener(bindings = @QueueBinding(value = @Queue(value = Constants.QUEUE.SEND_SUBSCRIBE.NAME), exchange = @Exchange(value = "${messagedriven.exchange.direct}"), key = Constants.QUEUE.SEND_SUBSCRIBE.KEY))
