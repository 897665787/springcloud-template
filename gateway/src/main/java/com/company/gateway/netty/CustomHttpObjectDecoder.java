 ...
 */
@Slf4j
public abstract class CustomHttpObjectDecoder extends ByteToMessageDecoder {
     ...
     */
    protected CustomHttpObjectDecoder() {
    ...
    private static String[] splitInitialLineOld(AppendableCharSequence sb) {
        int aStart;
        int aEnd;
        int bStart;
        int bEnd;
    ...
    private static String[] splitInitialLine(AppendableCharSequence sb) {
		try {
    		...
    		while (start < length) {
    			int nStart = findNonSPLenient(sb, start);
    			int nEnd = findSPLenient(sb, nStart);
    			list.add(sb.subStringUnsafe(nStart, nEnd));
    			start = nEnd;
    		}
