...
@RocketMQMessageListener(
		...
		consumerGroup = BroadcastConstants.SYS_USER_LOGIN.SYS_LOGIN_RECORD_QUEUE
)
@Slf4j
@Conditional(RocketMQAutoConfiguration.RocketMQCondition.class)
public class SysUserLoginRecordConsumer implements RocketMQListener<MessageExt> {
	@Override
	public void onMessage(MessageExt messageExt) {
		Map<String, String> properties = messageExt.getProperties();
		String jsonStrMsg = new String(messageExt.getBody(), Charset.forName("UTF-8"));

		properties.put(HeaderConstants.HEADER_STRATEGY_NAME, StrategyConstants.SYS_LOGINRECORD_STRATEGY);
