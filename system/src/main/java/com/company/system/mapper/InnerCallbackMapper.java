...
public interface InnerCallbackMapper extends BaseMapper<InnerCallback> {
    ...
    @Update("update bu_system_inner_callback set status = #{status.code},json_result = #{jsonResult},remark = #{remark} where id = #{id}")
    ...
    @Update("update bu_system_inner_callback set status = #{status.code},next_dispose_time = #{nextDisposeTime},failure = failure + 1,remark = #{remark} where id = #{id}")
    ...
    @Update("update bu_system_inner_callback set status = #{status.code} where id = #{id}")
    ...
    @Select("select id from bu_system_inner_callback where next_dispose_time <= now() and failure < max_failure and status = #{status}")
