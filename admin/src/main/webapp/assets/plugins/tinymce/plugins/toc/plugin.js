...
var toc = (function () {
  ...
  var $_9xm6cpsejn6umnmc = { create: create };

  var tocId = $_9xm6cpsejn6umnmc.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_47m4jysdjn6umnmb.getTocClass(editor);
    var headerTag = $_47m4jysdjn6umnmb.getTocHeader(editor);
    var selector = generateSelector($_47m4jysdjn6umnmb.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_47m4jysdjn6umnmb.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_47m4jysdjn6umnmb.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_dbadzrs9jn6umnm7.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_dbadzrs9jn6umnm7.updateToc(editor);
    });
  };
  var $_exnxavs8jn6umnm5 = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_47m4jysdjn6umnmb.getTocClass(editor);
  ...
  var $_8dg7o5sfjn6umnmd = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_dbadzrs9jn6umnm7.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_47m4jysdjn6umnmb.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_g1d3n4sgjn6umnmf = { register: register$1 };

  global.add('toc', function (editor) {
    $_exnxavs8jn6umnm5.register(editor);
    $_g1d3n4sgjn6umnmf.register(editor);
    $_8dg7o5sfjn6umnmd.setup(editor);
  });
  function Plugin () {
  }
