...
var textpattern = (function () {
  ...
  var $_4q74s7rwjn6umnkf = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_fbou5os4jn6umnkq.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_fbou5os4jn6umnkq.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_fbou5os4jn6umnkq.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_e1zerks1jn6umnkl.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_e1zerks1jn6umnkl.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_e1zerks1jn6umnkl.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_32e0pis0jn6umnki.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_32e0pis0jn6umnki.checkKeyCode(keyCodes, e)) {
        $_32e0pis0jn6umnki.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_32e0pis0jn6umnki.checkCharCode(charCodes, e)) {
  ...
  var $_8fivoorxjn6umnkg = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_4q74s7rwjn6umnkf.getPatterns(editor.settings));
    $_8fivoorxjn6umnkg.setup(editor, patternsState);
    return $_1qhbo9rvjn6umnke.get(patternsState);
  });
  function Plugin () {
  }
