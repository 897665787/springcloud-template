...
var searchreplace = (function () {
  ...
  var $_a0yoxkk4jn6umlaj = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_a0yoxkk4jn6umlaj.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_6kxdkhk2jn6umlae.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_6kxdkhk2jn6umlae.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_6kxdkhk2jn6umlae.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_6kxdkhk2jn6umlae.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_6kxdkhk2jn6umlae.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_6kxdkhk2jn6umlae.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_6kxdkhk2jn6umlae.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_6kxdkhk2jn6umlae.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_6kxdkhk2jn6umlae.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_6kxdkhk2jn6umlae.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_6kxdkhk2jn6umlae.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_6kxdkhk2jn6umlae.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_6kxdkhk2jn6umlae.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_6kxdkhk2jn6umlae.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_6kxdkhk2jn6umlae.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_c6rw4rk6jn6umlao = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_c6rw4rk6jn6umlao.open(editor, currentIndexState);
    });
  };
  var $_5kdc7uk5jn6umlam = { register: register };
  ...
  var $_b6ysvwk7jn6umlar = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_5kdc7uk5jn6umlam.register(editor, currentIndexState);
    $_b6ysvwk7jn6umlar.register(editor, currentIndexState);
    return $_2g2s2bk1jn6umlab.get(editor, currentIndexState);
  });
  function Plugin () {
  }
