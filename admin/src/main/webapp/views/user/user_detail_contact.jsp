            ...
            <div class="col-xs-12">
                <form class="form-horizontal" id="searchForm">
                    <div class="form-group">
						...
						<div class="col-xs-4 col-md-2 col-lg-1 no-padder m-b-md text-right">
							<label class="control-label">存在于通讯录：</label>
						</div>
						<div class="col-xs-8 col-md-4 col-lg-3  m-b-md">
							<select name="inBook" class="form-control" data-value="${search.inBook}">
								<option value="">全部</option>
								<xs:descriptionOptions clazz="user.UserContact" property="inBook"/>
							</select>
						</div>
                    </div>
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <%--<sec:authorize access="hasAnyRole(${xs:getPermissions('user_userContact_create')})">--%>
                    ...
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <c:forEach items="${pageModel.list}" var="item">
                            <tr>
								...
								<td>${item.name}</td>
								<td>${item.mobile}</td>
								<td><xs:descriptionDesc clazz="user.UserContact" property="inBook" value="${item.inBook}"/></td>
                                <td>${item.remark}</td>
                                <td>
            ...
            <div class="modal-content">
                ...
                <form name="createForm" class="form-horizontal" style="max-width: 800px">
                    <div class="modal-body">
						...
						<div class="form-group row">
							<div class="col-xs-3 text-right">
								<label class="control-label required">存在于通讯录：</label>
							</div>
							<div class="col-xs-9">
								<select name="inBook" class="form-control" data-ignore="true">
									<xs:descriptionOptions clazz="user.UserContact" property="inBook"/>
								</select>
							</div>
						</div>
                        <div class="form-group row">
                            ...
                            <div class="col-xs-9">
                                <input name="remark" type="text" maxlength="65535" class="form-control">
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        var createValidator = $createForm.validate({
			...
			messages: {
				...
				inBook: {
					required: "通讯录中是否存在不能为空"
				}
			},
            submitHandler: function () {
                $createSubmit.attr("disabled", true);
                doPost("<%=request.getContextPath()%>/admin/user/userContact/save",
                    $createForm.xsJson(),
                    function (data) {
                ...
                <form name="updateForm" class="form-horizontal" style="max-width: 800px">
                    <div class="modal-body">
						...
						<div class="form-group row">
							...
							<div class="col-xs-9">
								<select name="inBook" class="form-control">
									<xs:descriptionOptions clazz="user.UserContact" property="inBook"/>
								</select>
							</div>
						</div>
    ...
    <script>
        ...
        var updateValidator = $updateForm.validate({
            ...
            submitHandler: function () {
                ...
                doPost("<%=request.getContextPath()%>/admin/user/userContact/update", params,
                    function (data) {
        ...
        function updateListItem(id) {
            doPost("<%=request.getContextPath()%>/admin/user/userContact/get", {
                id: id,
                userId: '${user.id}'
            }, function (data) {
                if (data.status) {
    ...
    <script>
        function deleteListItem(id) {
            showDeleteModel(null, function () {
                doPost("<%=request.getContextPath()%>/admin/user/userContact/remove", {
                    id: id,
                    userId: '${user.id}'
                }, function (data) {
                    if (data.status) {
