...
<c:set var="index" value="marketing_wallet"/>
<%@include file="../common/content_nav.jsp" %>

<div class="app-content">
    <div class="app-content-body">
        <div class="bg-light lter b-b wrapper-md ">
            <h1 class="m-n font-thin h3 inline">钱包套餐管理</h1>
        </div>
                    ...
                    <div class="form-group">
						...
						<div class="col-xs-8 col-md-4 col-lg-3  m-b-md">
							<select name="status" class="form-control">
								...
								<xs:descriptionOptions clazz="user.wallet.WalletPackage" property="status" value="${search.status}"/>
							</select>
						</div>
						<div class="col-xs-4 col-md-2 col-lg-1 no-padder m-b-md text-right">
						...
						<div class="col-xs-8 col-md-4 col-lg-3  m-b-md">
							<select name="platform" class="form-control">
								<option value="">全部</option>
								<xs:descriptionOptions clazz="user.wallet.WalletPackage" property="platform" value="${search.platform}"/>
							</select>
						</div>
                    </div>
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${xs:getPermissions('wallet_walletPackage_create')})">
                    ...
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <c:forEach items="${pageModel.list}" var="item">
                            <tr>
								...
								<td><xs:descriptionDesc clazz="user.wallet.WalletPackage" property="status" value="${item.status}"/></td>
								<td>${item.seq}</td>
								<td><xs:descriptionDesc clazz="user.wallet.WalletPackage" property="platform" value="${item.platform}"/></td>
                                <td>
                                    <sec:authorize access="hasAnyRole(${xs:getPermissions('wallet_walletPackage_update')})">
                                        <a href="#" onclick="updateListItem('${item.id}');return false;"
                                           class="btn btn-info btn-xs">
                                            编辑
                                        </a>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${xs:getPermissions('wallet_walletPackage_delete')})">
						...
						<div class="form-group row">
							...
							<div class="col-xs-9">
								<select name="status" class="form-control" data-ignore="true">
									<xs:descriptionOptions clazz="user.wallet.WalletPackage" property="status"/>
								</select>
							</div>
						</div>
						<div class="form-group row">
						...
						<div class="form-group row">
							...
							<div class="col-xs-9">
								<select name="platform" class="form-control" data-ignore="true">
									<xs:descriptionOptions clazz="user.wallet.WalletPackage" property="platform"/>
								</select>
							</div>
						</div>
                    </div>
    ...
    <script>
        ...
        var createValidator = $createForm.validate({
            ...
            submitHandler: function () {
                ...
                doPost("<%=request.getContextPath()%>/admin/user/walletPackage/save",
    ...
    <script>
        ...
        var updateValidator = $updateForm.validate({
            ...
            submitHandler: function () {
                ...
                doPost("<%=request.getContextPath()%>/admin/user/walletPackage/update", params,
                    function (data) {
        ...
        function updateListItem(id) {
            doPost("<%=request.getContextPath()%>/admin/user/walletPackage/get", {id: id}, function (data) {
                if (data.status) {
    ...
    <script>
        function deleteListItem(id) {
            showDeleteModel(null, function () {
                doPost("<%=request.getContextPath()%>/admin/user/walletPackage/remove", {id: id}, function (data) {
                    if (data.status) {
