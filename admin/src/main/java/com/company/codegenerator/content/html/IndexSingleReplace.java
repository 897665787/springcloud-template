...
public class IndexSingleReplace implements Replace {
	
	List<Map<String, String>> datas = null;
	String tab;
	
	public IndexSingleReplace(String tab){
	...
	public String doReplace(String src) {
		...
		StringBuilder update_form = new StringBuilder();
		StringBuilder update_image_js = new StringBuilder();
		...
		String search_form_daterange_tpl = FileUtil.readString("classpath:code-generator-template/jsp/search/search_form_timerange.jsp", Charset.forName("utf-8"));
		...
		String input_form_num_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/input_form_num.jsp", Charset.forName("utf-8"));
		String input_form_select_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/input_form_select.jsp", Charset.forName("utf-8"));
		String input_form_text_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/input_form_text.jsp", Charset.forName("utf-8"));
		String save_form_image_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/save_form_image.jsp", Charset.forName("utf-8"));
		String update_form_image_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/update_form_image.jsp", Charset.forName("utf-8"));
		String update_image_tpl = FileUtil.readString("classpath:code-generator-template/jsp/indexSingle/update_image.js", Charset.forName("utf-8"));
		...
		for (Map<String, String> map : datas) {
			...
			if (!selectList.isEmpty()) {
				...
				table_td.append(table_td_select_tpl.replace("{columnName}", columnName).replace("{module}", Config.get("module.name")));
				...
				update_form.append(input_form_select_tmp);
				...
				String save_form_image_tmp = save_form_image_tpl
						.replace("{required}", required)
						.replace("{columnName}", columnName)
						.replace("{ColumnName}", ColumnName)
						.replace("{simpleColumnComment}", simpleColumnComment);
				save_form.append(save_form_image_tmp);
				
				String update_form_image_tmp = update_form_image_tpl
						.replace("{required}", required)
						.replace("{columnName}", columnName)
						.replace("{ColumnName}", ColumnName)
						.replace("{simpleColumnComment}", simpleColumnComment);
				update_form.append(update_form_image_tmp);
				
				String update_image_tmp = update_image_tpl
						.replace("{columnName}", columnName)
						.replace("{ColumnName}", ColumnName);
				update_image_js.append(update_image_tmp);
				
			} else if (Constants.dateSet.contains(dataType)) {
				// 日期框
				String search_form_daterange_tmp = search_form_daterange_tpl
				.replace("{columnName}", columnName)
				.replace("{simpleColumnComment}", simpleColumnComment);
				search_form.append(search_form_daterange_tmp);
				...
				update_form.append(input_form_text_tmp);
				...
				update_form.append(input_form_num_tmp);
			} else {
				// 文本框
				String search_form_text_tmp = search_form_text_tpl
				.replace("{columnName}", columnName)
		...
		src = src.replace("{update_form}", update_form.toString());
		src = src.replace("{update_image_js}", update_image_js.toString());
