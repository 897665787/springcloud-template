...
public class ModelReplace implements Replace {
	
	List<Map<String, String>> datas = null;
	String tab;
	
	public ModelReplace(String tab){
	...
	public String doReplace(String src) {
		...
		String field_create_update_tpl = FileUtil.readString("classpath:code-generator-template/java/model/field_create_update.java", Charset.forName("utf-8"));
		...
		for (Map<String, String> map : datas) {
			...
			if (Constants.ID.equalsIgnoreCase(column_name)) {
			...
			} else if (Constants.ignoreFieldSet.contains(column_name)) {
				String field_create_update_tmp = field_create_update_tpl;
				...
				String fieldfill = "INSERT";
				if (column_name.contains("update")) {
					fieldfill = "INSERT_UPDATE";
				}
				field_create_update_tmp = field_create_update_tmp.replace("{fieldfill}", fieldfill);
